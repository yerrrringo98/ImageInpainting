...
class EdgeConnect():
    def __init__(self, config):
        ...
        if self.config['MODE'] == 2:
            self.test_dataset = VGdataset(vocab=vocab, h5_path=self.config['test_h5_path'],
                                      image_dir=self.config['image_dir'], image_size=self.config['INPUT_SIZE'], include_relationships=False)
        else:
            self.train_dataset = VGdataset(vocab=vocab, h5_path=self.config['train_h5_path'],
                                      image_dir=self.config['image_dir'], image_size=self.config['INPUT_SIZE'], include_relationships=False)
            self.val_dataset = VGdataset(vocab=vocab, h5_path=self.config['val_h5_path'],
                                      image_dir=self.config['image_dir'], image_size=self.config['INPUT_SIZE'], include_relationships=False)
            self.sample_iterator = self.val_dataset.create_iterator(self.config['SAMPLE_SIZE'])
    ...
    def train(self):
        ...
        train_loader = torch.utils.data.DataLoader(dataset=self.train_dataset, **loader_kwargs)
        ...
        while(keep_training):
            ...
            for items in iter(train_loader):
                self.edge_model.train()
                self.inpaint_model.train()

                ground_truth, objs, bboxes, triples, masks, obj_to_img, triple_to_img = items
                items = load_data(ground_truth, masks, sigma=self.config['SIGMA'], mode='train', augment=False)
                ...
                if self.config['SAMPLE_INTERVAL'] and iteration % self.config['SAMPLE_INTERVALs'] == 0:
                    self.sample()

                # evaluate model at checkpoints
                if self.config['EVAL_INTERVALs'] and iteration % self.config['EVAL_INTERVAL'] == 0:
                    print('\nstart eval...\n')
                    self.eval()
