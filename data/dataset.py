...
class VGdataset(Dataset):
  def __init__(self, vocab, h5_path, image_dir, image_size=128,
                max_objects=5, max_samples=None,
    ...
    transform = [T.Resize((image_size,image_size)), T.ToTensor()]
  ...
  def __getitem__(self, index):
    ...
    # only one mask per images
    objs = objs[0].unsqueeze(0)
    boxes = boxes[0].unsqueeze(0)
    masks = masks[0].unsqueeze(0)
    triples = triples[0].unsqueeze(0)
    return image, objs, boxes, triples, masks
...
def vg_collate_fn(batch):
  ...
  all_imgs, all_objs, all_boxes, all_triples, all_masks = [], [], [], [], []
  all_obj_to_img, all_triple_to_img = [], []
  obj_offset = 0
  for i, (img, objs, boxes, triples, masks) in enumerate(batch):
