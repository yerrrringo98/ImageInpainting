...
def main(args):
  ...
  with open(args.images_json, 'r') as f:   #image_data.json
    images = json.load(f)
  image_id_to_image = {i['image_id']: i for i in images}

  with open(args.splits_json, 'r') as f:  #json for splitting
    splits = json.load(f)

  # Filter images for being too small
  splits = remove_small_images(args, image_id_to_image, splits)   #remove images which is too small
  ...
  with open(args.objects_json, 'r') as f:   # object_json
    objects = json.load(f)
  ...
  object_id_to_obj = filter_objects(args, objects, obj_aliases, vocab, splits)  # object id: object name, name_idx, object_box
