...
def local_patch2(x, bbox_list, image_size):
    assert len(x.size()) == 4
    H = image_size
    W = image_size
    patches = torch.zeros((x.size(0), 3, 64, 64)).cuda(0)
    for i, bbox in enumerate(bbox_list):
        bbox = bbox.squeeze()
        x0, y0, x1, y1 = bbox
        patch = x[i, :, int(y0 * H):int(y1 * H), int(x0 * W):int(x1 * W)]
        # x_, y, w, h = int(x_), int(y), int(w), int(h)
        # patch = x[i, :, x_:x_+w, y:y+h]
        patches[i] = T.Resize((64,64))(patch)
...
def mask_image2(gt, bboxes, config):
    height, width, _ = config['image_shape']
    batch_size = bboxes.size(0)
    mask = torch.zeros((batch_size, 1, height, width), dtype=torch.float32)
    for i in range(batch_size):
...
def spatial_discounting_mask(config):
    ...
    height = config['mask_shape']
    width = config['mask_shape']
